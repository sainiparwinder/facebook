...
if(isset($_POST['show_Friend'])){
		$x=$_SESSION["uid"];		
		$q="SELECT *  FROM `friend_requests` WHERE to_id=$x && friend=1";
		$data=mysqli_query($con,$q);
		$p='';
		if(mysqli_num_rows($data) > 0){		    
		      while ($row = mysqli_fetch_array($data)) { 
		         $p .='<tr>'.friendList($row["from_id"]).'</tr><br>'; 
		      }
		}
		$q="SELECT *  FROM `friend_requests` WHERE from_id=$x && friend=1";
		$data=mysqli_query($con,$q);			
	    if(mysqli_num_rows($data) > 0){			    
			  while ($row = mysqli_fetch_array($data)) { 
				 $p .='<tr> <td>'.friendList($row["to_id"]).'</td><br>';   
              } 
        }      echo $p; 
